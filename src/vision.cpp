...
ContourCorners getContourCorners(std::vector<cv::Point>& contour) {
	...
	double fittingError=.02;
    approxPolyDP(contour,approx,cv::arcLength(contour,true)*fittingError,true);
    std::cout << approx.size() << " " << "points generated by approxPolyDP" <<std::endl;
	if(approx.size()!=4){
		std::cout << "approxPolyDP generated something that's possibly garbage. Be forewarned." << std::endl;
	}
	for(auto i: approx){
		short t_s=0,r_s=0;
		for(auto j: approx){
